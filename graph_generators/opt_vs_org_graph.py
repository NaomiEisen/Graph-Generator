...
def org_vs_opt_bar_graph(data_org, data_opt, graph_config, file_name, test_name):
    ...
    # Plot the bars
    ax.bar(x - graph_config.BAR_WIDTH /2, data_org.mean(), graph_config.BAR_WIDTH, label='Original xml', color=LIGHT_BLUE)
    ax.bar(x + graph_config.BAR_WIDTH / 2, data_opt.mean(), graph_config.BAR_WIDTH, label='Optimal xml', color=LIGHT_ORANGE)
